...
-- Select query to view a listing of books excluded from the users wishlsit. User Id will change depending on which user is accessing
SELECT book_id, book_name, author, details
FROM book
WHERE book_id NOT IN (SELECT book_id FROM wishlist WHERE user_id = 1);

-- Insert statement to add a new book to the user's wishlist. The values will need to be replaced with the actual values the user selects from the program.
INSERT INTO wishlist(user_id, book_id)
    VALUES(1, 9)

-- Remove a book from the user's wishlist. The user_id and book_id values will need to be replaced with the actual values the user selects from program.
